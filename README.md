...
In order to prevent overfitting, we use a dropout rate of 0.5 in each layer. Finally, after the training converges, 
we evaluate our classifier using our test dataset. The performance of the model is measured using accuracy (74.2%) and the F1 score (74.2%). 
These results indicate that our model is able to capture the correlation between input parameters and the ground truth and, thus, 
...
- **Python Libraries:** `scikit-learn`, `PySR`, `numpy`, `pandas`, `matplotlib`
