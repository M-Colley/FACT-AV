...
def encode_scenario(scenario):
    ...
    return torch.nn.functional.one_hot(torch.tensor(classes.index(scenario)), num_classes=len(classes))

def encode_intro(intro):
    classes = ['ambigious', 'boasting']
    return torch.tensor(classes.index(intro))

def encode_cat(string, num_classes):
    classes = ["A1", "A2", "A3", "A4", "A5", "A6"]
    return torch.nn.functional.one_hot(torch.tensor(classes.index(string)), num_classes=num_classes)
...
class TrustDataset(Dataset):
    def __init__(self, file_path, split) -> None:
        ...
        x_values_extended = np.hstack([x_values, x_scenario, x_intro, x_gender, x_age, x_education, x_job, x_license, x_drivingfreq, x_distance, y_values])
        ...
        self.labels, self.counts = np.unique([int(d[-1]-1) for d in self.datapoints], return_counts=True)
        ...
        plt.yticks([])
        plt.savefig(f"{self.split}.labels.pdf")
        plt.savefig(f"{self.split}.labels.jpg")
        plt.close()
    ...
    def __getitem__(self, index):
        ...
        x = torch.concatenate([x_mIoU.view(1, -1), 
                               x_scenario.view(1, -1), 
                               x_intro.view(1, -1), 
                               x_gender.view(1, -1), 
                               x_age.view(1, -1), 
